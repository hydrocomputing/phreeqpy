...
def testallmethods_py():
    ...
    yrm.YAMLSetDumpFileName("all_methods.dump")
    yrm.YAMLSetFilePrefix("all_methods")
    yrm.YAMLOpenFiles()
    ...
    yrm.YAMLSetTimeConversion(1.0/86400.0)
    yrm.YAMLSetRepresentativeVolume(np.ones(nxyz))
    yrm.YAMLSetPrintChemistryMask(np.ones(nxyz, dtype=int))
    ...
    yrm.YAMLAddOutputVars("AddOutputVars", "True")
    yrm.YAMLAddOutputVars("SolutionProperties", "True")
    yrm.YAMLAddOutputVars("SolutionTotalMolalities", "True")
    yrm.YAMLAddOutputVars("ExchangeMolalities", "True")
    yrm.YAMLAddOutputVars("SurfaceMolalities", "True")
    yrm.YAMLAddOutputVars("EquilibriumPhases", "True")
    yrm.YAMLAddOutputVars("Gases", "True")
    yrm.YAMLAddOutputVars("KineticReactants", "True")
    yrm.YAMLAddOutputVars("SolidSolutions", "True")
    yrm.YAMLAddOutputVars("CalculateValues", "True")
    yrm.YAMLAddOutputVars("SolutionActivities", "H+ Ca+2 Na+")
    yrm.YAMLAddOutputVars("SolutionMolalities", "OH- Cl-")
    yrm.YAMLAddOutputVars("SaturationIndices", "Calcite Dolomite")
    # Remove any reactants in workers
    yrm.YAMLRunString(True, False, False, "DELETE; -all")
    # Transfer initial conditions
    ones_int = np.ones(nxyz, dtype=int)
    yrm.YAMLInitialEquilibriumPhases2Module(ones_int)
    yrm.YAMLInitialExchanges2Module(ones_int)
    yrm.YAMLInitialGasPhases2Module(ones_int)
    yrm.YAMLInitialKinetics2Module(ones_int)
    yrm.YAMLInitialSolutions2Module(ones_int)
    yrm.YAMLInitialSolidSolutions2Module(ones_int)
    yrm.YAMLInitialSurfaces2Module(ones_int)
    # Alternative A. to the previous seven methods
    # yrm.YAMLInitialPhreeqc2Module(np.ones(nxyz * 7, dtype=int))
    # Alternative B. to the previous seven methods
    # v1 = np.full(nxyz*7, 1)
    # v2 = np.full(nxyz*7, -1)
    # f1 = np.full(nxyz*7, 1.0)
    # yrm.YAMLInitialPhreeqc2Module_mix(v1, v2, f1)
    # Alternative C. to the previous seven methods
    # yrm.YAMLInitialPhreeqcCell2Module(1, np.array([18, 19]))
    ...
    YAML_filename = "all_methods.yaml"
